if (typeof window !== 'undefined' && window.implode) {
  return module.exports = window.implode
}
...
if (typeof window !== 'undefined') window.implode = implode
...
implode.recover.traverse = function traverse(val, ident, parent) {
  ...
  // if it is a class, rebuild it
    ...
    copy = new implode.contracts[val.$type]
    if ('arr' in val) {
      var obj = val.obj
      val = val.arr
