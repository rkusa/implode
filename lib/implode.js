var contracts = typeof window !== 'undefined'
  ? (window.implodeContracts ? window.implodeContracts : (window.implodeContracts = {}))
  : {}
...
implode.recover = function(obj) {
  ...
  function traverse(val, ident, parent) {
    ...
    //
      ...
      if (!val.$proxy.fn) {
        proxyQueue.push(function($proxy) {
          var proxied = proxy.apply(null, [$proxy.target, $proxy.method, $proxy.context].concat($proxy.args))
          if (ident === undefined) parent.push(proxied)
          else parent[ident] = proxied
        }.bind(null, traverse(val.$proxy, null, null)))
      } else {
        proxyQueue.push(function($proxy) {
          var proxied = proxy.apply(null, [$proxy.target, $proxy.context].concat($proxy.args))
          if (ident === undefined) parent.push(proxied)
          else parent[ident] = proxied
        }.bind(null, traverse(val.$proxy, null, null)))
      }
