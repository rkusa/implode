...
implode.recover = function(obj) {
  ...
  function traverse(val, ident, parent) {
    ...
    //
      ...
      if (!val.$proxy.fn) {
        queue.push((function($proxy) {
          traverse($proxy.target, ident, parent)
          queue.push(function(target) {
            parent[ident] = proxy.apply(null, [parent[ident], $proxy.method, $proxy.context].concat($proxy.args))
          })
        }).bind(null, val.$proxy))
      } else {
        queue.push((function($proxy) {
          traverse($proxy.fn, ident, parent)
          queue.push(function(target) {
            parent[ident] = proxy.apply(null, [parent[ident], $proxy.context].concat($proxy.args))
          })
        }).bind(null, val.$proxy))
      }
      return
